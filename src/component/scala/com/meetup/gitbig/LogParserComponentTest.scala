...
import com.meetup.gitbig.parser.{CommitParser, LogParser}
import org.scalatest.{FunSpec, Matchers}

import scala.collection.mutable
import scala.io.Source

class LogParserComponentTest extends FunSpec with Matchers {
...
5       6       util/db/MUP-3626_schema_update.sql"""
  ...
  describe("LogParser") {
    it("should parse a log to commits") {
      val expected = List(
        Commit(
      ...
      val outBuffer = mutable.ListBuffer[Commit]()
        def out(commit: Commit): Unit = {
          outBuffer += commit
        }

      new LogParser(CommitParser).parse(Source.fromString(full), out)

      val actual = outBuffer.toList

      if (actual.size != expected.size) {
        fail(s"Lists of different sizes: expected: $expected, actual: $actual")
      }
